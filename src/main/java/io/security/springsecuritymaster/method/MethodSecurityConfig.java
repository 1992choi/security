...
public class MethodSecurityConfig {
    ...
    public Advisor protectServicePointcut2() {
        AspectJExpressionPointcut pointcut = new AspectJExpressionPointcut();
        pointcut.setExpression("execution(* io.security.springsecuritymaster.DataService.getAdmin(..))");
        AuthorityAuthorizationManager<MethodInvocation> manager = hasRole("ADMIN");
        return new AuthorizationManagerBeforeMethodInterceptor(pointcut, manager);
    }
    ...
    @Bean
    @Role(BeanDefinition.ROLE_INFRASTRUCTURE)
    public Advisor protectServicePointcut3() {
