        ...
        http.authorizeHttpRequests(auth -> auth.anyRequest().authenticated())
                .formLogin(form -> form
                        ...
                        .successHandler((request, response, authentication) -> { // 인증 성공 시 사용할 AuthenticationSuccessHandler를 지정. 기본값은 SavedRequestAwareAuthenticationSuccessHandler이다
                            ...
                            response.sendRedirect("/home"); // 핸들러를 명시할 경우, defaultSuccessUrl()보다 우선순위가 높게 동작한다.
                        })
                        .failureHandler((request, response, exception) -> { // 인증 실패 시 사용할 AuthenticationFailureHandler를 지정. 기본값은 SimpleUrlAuthenticationFailureHandler를 사용하여 "/login?error"로 리다이렉션 함
                            System.out.println("exception : " + exception.getMessage());
                            response.sendRedirect("/login"); // 핸들러를 명시할 경우, failureUrl()보다 우선순위가 높게 동작한다.
                        })
